...
class direccion:
    ...
    def __init__(self, error, tam_horiz_frame):
        self.rango_seguro_min = (tam_horiz_frame/2)-(tam_horiz_frame*error)
        x = [0,(self.rango_seguro_min/2),self.rango_seguro_min]
        y = [90,45,0]
        slope_1, intercept_1, r_value, p_value, std_err = stats.linregress(x, y)
        self.f_derecha = lambda i: float(intercept_1) + float(slope_1)*i

        self.rango_seguro_max = (tam_horiz_frame/2)+(tam_horiz_frame*error)
        x = [self.rango_seguro_max,
            self.rango_seguro_max+((tam_horiz_frame-self.rango_seguro_max)/2),
            tam_horiz_frame]
        y = [0,45,90]
        slope_2, intercept_2, r_value, p_value, std_err = stats.linregress(x, y)
